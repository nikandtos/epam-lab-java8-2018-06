...
public class Exercise1 {
    ...
    public void findAllCompanies() {
        ...
        Set<String> companies = employees.stream().flatMap(employee -> employee.getJobHistory().stream().map(e->e.getEmployer())).collect(
            Collectors.toSet());
    ...
    public void findMinimalAgeOfEmployees() {
        ...
        // TODO реализация
        Optional<Person> min = employees.stream().map(Employee::getPerson)
                                        .min(Comparator.comparingInt(Person::getAge));
        Integer minimalAge = min.get().getAge();
